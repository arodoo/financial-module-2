    ...
    if (isset($_POST['save_payment']) || isset($_POST['update_payment']) || isset($_POST['delete_payment'])) {
        ...
        if (isset($_POST['save_payment'])) {
            ...
            if (empty($_POST['name']) || empty($_POST['category_id']) || empty($_POST['amount'])) {
                ...
                if ($result) {
                    $flashMessage = 'Paiement enregistré avec succès! (ID: ' . $result . ')';
                    $flashType = 'success';
                } else {
    ...
    elseif (isset($_POST['save_expense']) || isset($_POST['update_expense']) || isset($_POST['delete_expense'])) {
        ...
        if (isset($_POST['save_expense'])) {
            ...
            if (empty($_POST['name']) || empty($_POST['category_id']) || empty($_POST['amount'])) {
                ...
                if ($result) {
                    $flashMessage = 'Dépense enregistrée avec succès! (ID: ' . $result . ')';
                    $flashType = 'success';
                } else {
...
window.submitPaymentForm = function(event) {
    ...
    .catch(error => {
        ...
        showNotification('Erreur lors de la mise à jour du paiement', 'danger');
    });
};
...
function showNotification(message, type = 'success') {
    ...
    if (!container) {
        ...
        container.style.zIndex = '1060'; // Higher than modal backdrop
        container.style.marginTop = '60px'; // Add some top margin
    ...
    notification.style.minWidth = '300px'; // Set a minimum width
    notification.style.boxShadow = '0 0.5rem 1rem rgba(0, 0, 0, 0.15)'; // Add shadow
...
function reloadExpensesData() {
    ...
    const expensesContainer = document.querySelector('#expenses .col-md-7');
    if (!expensesContainer) return;
    
    expensesContainer.innerHTML = '<div class="text-center py-5"><div class="spinner-border text-primary" role="status"><span class="visually-hidden">Loading...</span></div><p class="mt-2">Chargement des données...</p></div>';
    
    // Extract the base URL without query parameters
    const baseUrl = window.location.href.split('?')[0];
    const fetchUrl = baseUrl + '?action=fixed-payments&ajax=1&get_expenses=1';
    ...
    fetch(fetchUrl)
        ...
        .then(html => {
            // Simple full page reload is more reliable
            location.reload();
        })
        .catch(error => {
            console.error('Error reloading expenses:', error);
            expensesContainer.innerHTML = '<div class="alert alert-danger">Erreur lors du chargement des données.</div>';
        });
}
